...
export const metadata: Metadata = { title: "Commodities Intelligence" };

export default function CommoditiesPage() {
  const signals = MOCK_SIGNALS.filter((s) => s.assetClass === "COMMODITIES");
  return (
    <div style={{ padding: "var(--space-8) var(--space-10)" }}>
      <div style={{ marginBottom: "var(--space-6)", paddingBottom: "var(--space-6)", borderBottom: "1px solid var(--border)" }}>
        ...
        <h1 style={{ fontSize: "1.25rem", fontWeight: 500, color: "var(--text-primary)", marginBottom: "var(--space-1)" }}>Commodities Intelligence</h1>
        <p style={{ fontSize: "0.8125rem", color: "var(--text-muted)" }}>{signals.length} active signals · Precious metals, energy and industrials</p>
      </div>
